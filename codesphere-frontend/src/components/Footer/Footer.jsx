...
// "Amanah.IT" ke split kore "Amanah." white r "IT" green kore dekhay
function DevelopedByName({ name }) {
    if (name.includes('.')) {
        const [base, accent] = name.split('.');
        return (
            <>
                <span style={{ color: '#fff' }}>{base}.</span>
                <span style={{ color: '#22c55e' }}>{accent}</span>
            </>
        );
    }
    return <span style={{ color: '#fff' }}>{name}</span>;
}
                ...
                <div className="footer-copyright" style={{ textAlign: 'center', padding: '18px 0', fontSize: '13px', color: 'rgba(255,255,255,0.55)', borderTop: '1px solid rgba(255,255,255,0.9)' }}>
                    ...
                    {f.developedByLink ? (
                        <a href={f.developedByLink} target="_blank" rel="noopener noreferrer" style={{ fontWeight: 600, textDecoration: 'none' }}>
                            <DevelopedByName name={f.developedByName} />
                        </a>
                    ) : (
