...
export default function ServicesTab() {
    ...
    const loadSections = () => {
        api.get('/home-sections').then(r => {
            ...
            if (d.servicesPageHeader) {
                ...
                if (nonEmpty.heroImage) setHeroImagePreview(nonEmpty.heroImage);
            }
            if (d.servicesPageBenefits?.items?.length) setBenefits({ ...DEFAULT_BENEFITS, ...d.servicesPageBenefits });
            if (d.servicesPageCtaStrip) setCtaStrip({ ...DEFAULT_CTA_STRIP, ...d.servicesPageCtaStrip });
            if (d.servicesPageBottomCta) setServicesBottomCta({ ...DEFAULT_SERVICES_BOTTOM_CTA, ...d.servicesPageBottomCta });
    ...
    // ---- Detail Benefits banner image upload ----
    // ---- Services page hero image upload ----
    const handleHeroImage = (e) => {
        ...
        reader.onload = () => {
            ...
            setPageHeader({ ...pageHeader, heroImage: reader.result });
        };
        reader.readAsDataURL(file);
    };
    const removeHeroImage = () => {
        setHeroImagePreview('');
        setPageHeader({ ...pageHeader, heroImage: '' });
    };
                        ...
                        <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '0.75rem' }}>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 400 }}>
                                <input
                                    type="radio"
                                    name="heroBgType"
                                    checked={pageHeader.heroBgType !== 'image'}
                                    onChange={() => setPageHeader({ ...pageHeader, heroBgType: 'gradient' })}
                                />
                                Gradient (default green banner)
                            </label>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 400 }}>
                                <input
                                    type="radio"
                                    name="heroBgType"
                                    checked={pageHeader.heroBgType === 'image'}
                                    onChange={() => setPageHeader({ ...pageHeader, heroBgType: 'image' })}
                                />
                                Image
                            </label>
                        </div>

                        {pageHeader.heroBgType === 'image' && (
                            <div>
                                {heroImagePreview && (
                                    <div style={{ marginBottom: 8 }}>
                                        <img src={heroImagePreview} alt="hero preview" style={{ width: '100%', maxWidth: 400, borderRadius: 8, display: 'block', marginBottom: 6 }} />
                                        <button type="button" className="admin-btn admin-btn-outline admin-btn-sm" onClick={removeHeroImage}>
                                            <FiX size={13} /> Remove Image
                                        </button>
                                    </div>
                                )}
                                <label className="admin-btn admin-btn-outline admin-btn-sm" style={{ width: 'fit-content', cursor: 'pointer' }}>
                                    <FiUpload size={14} /> Upload Hero Image
                                    <input type="file" accept="image/*" onChange={handleHeroImage} style={{ display: 'none' }} />
                                </label>
                            </div>
                        )}
                    </div>
